    ...
    <>
      ...
      <FlexBox sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' }, gap: 2 }}>
        ...
        <SectionCard>
          <FlexBox sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
            <Typography variant="subtitle2" sx={{ fontWeight: 700 }}>
              Assign preprocessing to selected columns
            </Typography>
            <Typography variant="caption" color="textSecondary">
              Configure methods below, then click “Apply to Selected”. This creates per-column overrides.
            </Typography>

            {/* Optional: allow user to set defaults directly when tasks enabled */}
            <FlexBox
              sx={{
                border: '1px solid rgba(0,0,0,0.08)',
                borderRadius: 2,
            ...
            >
              <Typography variant="caption" sx={{ fontWeight: 700 }}>
                Defaults (applied to all columns of a type)
              </Typography>

              <FlexBox sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1fr 1fr' }, gap: 1, mt: 1 }}>
                {hasTask('handle_missing_categoricals') && (
                  <FlexBox sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                    <Typography variant="caption" color="textSecondary">
                      Categorical missing default
                    </Typography>
                    <SelectNative
                      value={defaults.categoricalMissing}
                      onChange={(v) => onDefaultsChange((p) => ({ ...p, categoricalMissing: v }))}
                      options={[
                ...
                {hasTask('numeric_imputation') && (
                  <FlexBox sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                    <Typography variant="caption" color="textSecondary">
                      Numeric missing default
                    </Typography>
                    <SelectNative
                      value={defaults.numericMissing}
                      onChange={(v) => onDefaultsChange((p) => ({ ...p, numericMissing: v }))}
                      options={[
                ...
                {hasTask('numeric_scaling') && (
                  <FlexBox sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                    ...
                    <SelectNative
                      ...
                      onChange={(v) => onDefaultsChange((p) => ({ ...p, scaling: v }))}
                      options={[
                ...
                {hasTask('encode_categoricals') && (
                  <FlexBox sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                    <Typography variant="caption" color="textSecondary">
                      One-hot max levels default
                    </Typography>
                    <InputNative
                      type="number"
                      value={defaults.oneHotMaxLevels}
                      onChange={(e) => onDefaultsChange((p) => ({ ...p, oneHotMaxLevels: Number(e.target.value) }))}
                    />
                  </FlexBox>
                )}

                {hasTask('reduce_cardinality') && (
                  <FlexBox sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                    <Typography variant="caption" color="textSecondary">
                      Rare threshold default
                    </Typography>
                    <InputNative
                      type="number"
                      step="0.001"
                      value={defaults.rarePropThreshold}
                      onChange={(e) => onDefaultsChange((p) => ({ ...p, rarePropThreshold: Number(e.target.value) }))}
                    />
                  </FlexBox>
                )}
                ...
                {hasTask('reduce_cardinality') && (
                  <FlexBox sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                    <Typography variant="caption" color="textSecondary">
                      High-card threshold default
                    </Typography>
                    <InputNative
                      type="number"
                      value={defaults.highCardinalityThreshold}
                      onChange={(e) =>
                        onDefaultsChange((p) => ({ ...p, highCardinalityThreshold: Number(e.target.value) }))
                      }
                    />
                  </FlexBox>
                )}
